    ...
    getStatistic() {
      axios.get('/ebook-snapshot/get-statistic').then((response) => {
        ...
        if (data.success) {
          ...
          this.statistic.viewCount = statisticResp[1].viewCount;
          this.statistic.voteCount = statisticResp[1].voteCount;
          this.statistic.todayViewCount = statisticResp[1].viewIncrease;
          this.statistic.todayVoteCount = statisticResp[1].voteIncrease;

          // 按分钟计算当前时间点，占一天的百分比
          const now = new Date();
          const nowRate = (now.getHours() * 60 + now.getMinutes()) / (60 * 24);
          // console.log(nowRate)
          this.statistic.todayViewIncrease = parseInt(String(statisticResp[1].viewIncrease / nowRate));
          // todayViewIncreaseRate：今日预计增长率
          this.statistic.todayViewIncreaseRate = (this.statistic.todayViewIncrease - statisticResp[0].viewIncrease) / statisticResp[0].viewIncrease * 100;
          this.statistic.todayViewIncreaseRateAbs = Math.abs(this.statistic.todayViewIncreaseRate);
        }
      });
    },
    get30DayStatistic() {
